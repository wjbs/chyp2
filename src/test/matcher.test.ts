...
import { Graph, gen, identity } from '../lib/graph.ts';
...
describe('Match.tryAddVertex()', () => {
    ...
    it('fails when cod vertex is boundary but dom vertex is not', () => {
        ...
        // h is a gen with a boundary vertex
        const h = gen('f', 1, 1);
        const m = new Match(g, h);
...
describe('Matches enumeration', () => {
    it('finds exactly one match of gen(f,1,1) into gen(f,1,1)', () => {
        const dom = gen('f', 1, 1);
        const cod = gen('f', 1, 1);
    ...
    it('finds a match of f in a composed graph f;g', () => {
        const f = gen('f', 1, 1);
        const fg = gen('f', 1, 1).compose(gen('g', 1, 1));
...
describe('matchRule()', () => {
    it('finds the LHS of a rule inside a larger graph', () => {
        ...
        const graph = gen('f', 1, 1).compose(gen('h', 1, 1));
    ...
    it('finds no matches when the rule LHS is absent', () => {
        const r = new Rule(gen('x', 1, 1), gen('y', 1, 1));
        const graph = gen('f', 1, 1).compose(gen('g', 1, 1));
...
describe('findIso()', () => {
    ...
    it('finds an isomorphism between two copies of a more complicated graph', () => {
        const g = gen('f', 1, 2).compose(gen('g', 2, 1));
        const h = gen('f', 1, 2).compose(gen('g', 2, 1));
        const iso = findIso(g, h);
        assert.ok(iso !== undefined);
    });
    ...
    it('finds an isomorphism for the identity graph', () => {
        const g = identity();
        const h = identity();
        const iso = findIso(g, h);
        assert.ok(iso !== undefined);
