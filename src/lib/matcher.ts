...
function matchLog(s: string): void {
    if (DEBUG_MATCH) console.log(s);
}
...
export class Match {
    ...
    constructor(m: Match);
    constructor(domOrMatch: Graph | Match, cod?: Graph) {
    ...
    domNhdMapped(v: number): boolean {
        for (const e of this.dom.inEdges(v)) {
            if (!this.emap.has(e)) return false;
        }
     ...
     */
    mapScalars(): boolean {
        const codScalars: Array<[number, unknown]> = [];
        ...
        for (const e of this.dom.edges()) {
            ...
            if (ed.s.length !== 0 || ed.t.length !== 0) continue;
            let found = false;
    ...
    more(): Match[] {
        ...
        for (const v of this.dom.vertices()) {
            if (this.vmap.has(v)) continue;
            for (const codV of this.cod.vertices()) {
                const m1 = this.copy();
                if (m1.tryAddVertex(v, codV)) ms.push(m1);
            }
            return ms;
        }
    ...
    isConvex(): boolean {
        if (!this.isInjective()) return false;
        ...
        for (const v of this.dom.inputs()) {
            if (this.vmap.has(v) && future.has(this.vmap.get(v)!)) return false;
        }
        return true;
    }
}
 ...
 */
export class Matches implements Iterable<Match>, Iterator<Match, undefined> {
    private matchStack: Match[];
    private readonly convex: boolean;

    constructor(dom: Graph, cod: Graph, initialMatch?: Match, convex: boolean = true) {
        if (initialMatch === undefined) initialMatch = new Match(dom, cod);
        this.convex = convex;
...
export function matchGraph(dom: Graph, cod: Graph, convex: boolean = true): Matches {
    return new Matches(dom, cod, undefined, convex);
}

/** Returns an iterable of all (convex) matches of the rule's LHS into `g` */
export function matchRule(r: Rule, g: Graph, convex: boolean = true): Matches {
 ...
 */
export function findIso(g: Graph, h: Graph): Match | undefined {
    const gIn = g.inputs();
    const gOut = g.outputs();
    const hIn = h.inputs();
    const hOut = h.outputs();

    if (gIn.length !== hIn.length || gOut.length !== hOut.length) return undefined;

    const m0 = new Match(g, h);
    for (let i = 0; i < gIn.length; i++) {
        if (!m0.tryAddVertex(gIn[i], hIn[i])) return undefined;
    }
    for (let i = 0; i < gOut.length; i++) {
        if (!m0.tryAddVertex(gOut[i], hOut[i])) return undefined;
    }

    for (const m of new Matches(g, h, m0, false)) {
        if (m.isSurjective()) return m;
    }
