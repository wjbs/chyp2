...
function getTextWidthFont(text : string, font : string) : number {
...
function getTextWidthWithDom(text : string) : number {
    const font : string = "normal sans-serif " + (0.3 * SCALE);
...
export function getTextWidth(s : string) : number {
...
export function mkBezierC(p1x : number, p1y : number, 
    cp1x : number, cp1y : number, cp2x : number, cp2y : number, 
    p2x : number, p2y : number) : string {
    return `M ${p1x} ${p1y} C ${cp1x} ${cp1y} ${cp2x} ${cp2y} ${p2x} ${p2y}`;

}

export function curveBetween(p1x : number, p1y : number, p2x : number, p2y : number) : string {
...
export function curveTo(p1x : number, p1y : number, p2x : number, p2y : number) : string {
    const cp1x = 0.6 * p1x + 0.4 * p2x;
    const cp1y = p1y;
...
export function vertexiyShift(i : number, num : number) {
    return (num <= 1 ? 0 : (i / (num - 1)) - 0.5);
}

export function vertexyShift(v : number, st : number[]) {
    const idx : number = st.indexOf(v);
    if (idx === -1) {
        return 0;
    }
    else {
