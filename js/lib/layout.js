...
function makeOptimizationProblem(g, eLayers, s) {
    ...
    // Constraints : 
    for (const [io, boundary] of [['i', g.inputs()], ['o', g.outputs()]]) {
        for (let i = 0; i < boundary.length - 1; i++) {
            const Arow = new Array(size).fill(0);
            const v1 = boundary[i];
...
function Q2Hessian(dimension, Q) {
...
function elementaryLayerDecompPreLayout(s, g, eLayers) {
    ...
    for (const eLayer of eLayers) {
        ...
        for (let i = 0; i < vLayer.length; i++) {
            ...
            vd.y = average([...vd.outEdges].map(e => g.edgeData(e).y + vertexyShift(v, g.edgeData(e).s)).concat([...vd.inEdges].map(e => g.edgeData(e).y + vertexyShift(v, g.edgeData(e).t)))) ?? 0;
            // i - (vLayer.length - 1) / 2;   
        }
 ...
 */
export function convexElementaryLayout(g, force = false, NUM_ITERATIONS = 10, s) {
    if (g.laidOut && !force)
        return;
    const eLayers = layerDecomp(g);
    let isPerm = true;
    ...
    for (let iter = 0; iter < NUM_ITERATIONS; iter++) {
        ...
        for (const vList of [g.inputs(), g.outputs()]) {
            ...
            const minGaps = new Array(vList.length - 1).fill(0.5);
            enforceMinSpacing(positions, minGaps);
