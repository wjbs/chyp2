...
export class Graph {
     ...
     */
    addVertex(x = 0, y = 0, value = '', name = -1) {
        let v;
        if (name === -1) {
            v = this.vindex;
            this.vindex += 1;
    ...
    addEdge(s, t, value = '', x = 0, y = 0, width = null, height = null, fg = '', bg = '', hyper = true, name = -1) {
        ...
        for (const v of t)
            this.vdata.get(v).inEdges.add(e);
        return e;
    }
    /**
     * Remove a vertex.
    ...
    tensor(other, layout = true) {
        ...
        if (layout) {
            ...
            for (const v of g.vertices())
                g.vertexData(v).y -= maxSelf;
            for (const e of g.edges())
                g.edgeData(e).y -= maxSelf;
        }
    ...
    compose(other) {
        ...
        for (const v of g.vertices())
            g.vertexData(v).x -= maxSelf;
        for (const e of g.edges())
            g.edgeData(e).x -= maxSelf;
        for (const v of other.vertices()) {
            const vd = other.vertexData(v);
        ...
        if (plug1.length !== plug2.length) {
            throw new GraphError(`Attempting to plug a graph with ${plug1.length} outputs into one with ${plug2.length} inputs`);
        }
        g.setOutputs(other.outputs().map(v => vmap.get(v)));
        for (let i = 0; i < plug1.length; i++) {
            let p1 = plug1[i];
            let p2 = plug2[i];
            while (quotient.has(p1))
                p1 = quotient.get(p1);
            while (quotient.has(p2))
                p2 = quotient.get(p2);
            if (p1 !== p2) {
...
export function graphFromJson(jsonString) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const j = JSON.parse(jsonString);
    const g = new Graph();
    for (const [v, vd] of Object.entries(j.vertices)) {
