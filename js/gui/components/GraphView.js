...
export function GraphView({ s, uuid, graph }) {
    ...
    convexLayout(s, graph, true, window.NUM_ITERATIONS ?? 10);
...
export function GraphPanels({ s, lhs, rhs }) {
    if (rhs === null) {
        return (_jsx("div", { style: { display: 'flex', width: '100%', height: '100%', justifyContent: 'center' }, children: _jsx(GraphView, { graph: lhs, uuid: "lhs", s: s }) }));
    }
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'row', width: '100%', height: '100%' }, children: [_jsx("div", { style: { flex: 1, borderRight: '1px solid #ccc' }, children: _jsx(GraphView, { graph: lhs, uuid: "lhs", s: s }) }), _jsx("div", { style: { flex: 1 }, children: _jsx(GraphView, { graph: rhs, uuid: "rhs", s: s }) })] }));
}
