...
const setPartsEffect = StateEffect.define();
const partsField = StateField.define({
    create() {
        ...
        for (const effect of tr.effects) {
            if (effect.is(setPartsEffect)) {
                ...
                effect.value.parts.forEach((part, i) => {
                    ...
                    const status = part.status === Part.CHECKING ? 'checking' :
...
export function CodeView({ state, currentPart, initialContent = '', onChange }) {
    ...
    useEffect(() => {
        ...
        if (onChange) {
            extensions.push(EditorView.updateListener.of((update) => {
                onChangeRef.current?.(update.docChanged ? update.state.doc.toString() : null, update.selectionSet ? update.state.selection.main.head : null);
            }));
        }
    ...
    return (_jsx("div", { className: "editor-panel", children: _jsx("div", { ref: containerRef }) }));
}
